...
void validate_verify_inputs(
  ...
  if (slot_ids.ndim() != 2 || slot_ids.dtype() != mx::int32) {
    throw std::runtime_error(
        std::string(op_name) + ": slot_ids must be int32 2D");
  }
  ...
  const int num_draft = slot_ids.shape(1);
  if (q.shape(0) != batch * num_draft) {
    throw std::runtime_error(
        std::string(op_name) + ": q.shape[0] must be batch * num_draft");
  }
}
...
mx::array gdn_state_verify(
  ...
  validate_verify_inputs(q, k, v, g, beta, state, slot_ids, "gdn_state_verify");
  ...
  return mx::array(
      {tokens, hv, dv},
      q.dtype(),
      std::make_shared<GDNStateVerify>(s, q.shape(1), v.shape(1), q.shape(2), v.shape(2)),
      {q, k, v, g, beta, state, slot_ids});
}

#ifdef _METAL_

void GDNStateVerify::eval_gpu(
  ...
  auto kernel = d.get_kernel("gdn_state_verify_float32_dk128_dv128", lib);
  auto& compute_encoder = d.get_command_encoder(s.index);

  const uint32_t batch_u = static_cast<uint32_t>(slot_ids.shape(0));
  const uint32_t num_draft_u = static_cast<uint32_t>(slot_ids.shape(1));
  ...
  compute_encoder.set_bytes(num_draft_u, 9);
...
void GDNStateVerify::eval_gpu(
  ...
  (void)inputs;
  (void)outputs;
  throw std::runtime_error("GDNStateVerify: Metal backend required");
}
