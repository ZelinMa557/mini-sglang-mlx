...
This module exposes the standard mini-sglang-mlx ``Model`` / ``ModelArgs``
pair so the generic :func:`mini_sglang_mlx.models.load_model` discovers
...
* Multi-layer attention with two kernel variants per layer:
  ...
  the underlying ``mini_sglang_mlx_kernel.paged_prefill_attention`` kernel
...
from mini_sglang_mlx.layers.rotary_embedding import RotaryEmbedding
...
def load_dflash_draft_model(
    ...
    :func:`mini_sglang_mlx.models.load_model`; the only extra step is
    ...
    Args:
        ...
        target_model: An already-loaded mini-sglang-mlx target model; its
    ...
    """
    from mini_sglang_mlx.models import load_model
