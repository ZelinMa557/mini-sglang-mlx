...
from mini_sglang_mlx.engine import SPEC_ALGOS
from mini_sglang_mlx.scheduler import SchedulerConfig
from mini_sglang_mlx.utils import cached_load_hf_config, init_logger, resolve_repo
...
class ServerArgs(SchedulerConfig):
    ...
    def zmq_frontend_addr(self) -> str:
        return "ipc:///tmp/mini_sglang_mlx_3" + self._unique_suffix

    @property
    def zmq_tokenizer_addr(self) -> str:
        if self.share_tokenizer:
            return self.zmq_detokenizer_addr
        result = "ipc:///tmp/mini_sglang_mlx_4" + self._unique_suffix
...
# normalise to lowercase to match :data:`mini_sglang_mlx.engine.SPEC_ALGOS`.
...
def parse_args(args: List[str]) -> ServerArgs:
    ...
    :func:`mini_sglang_mlx.utils.resolve_repo` so HF and ModelScope share
    a single code path.
    """
    from mini_sglang_mlx.kvcache import SUPPORTED_CACHE_MANAGER

    parser = argparse.ArgumentParser(description="mini-sglang-mlx Server Arguments")
