...
Limitations (intentional first cut):
    ...
    * No chunked prefill: :meth:`DflashEngine._run_prefill` raises on
      a :class:`~mini_sglang_mlx.scheduler.prefill.ChunkedReq`, so a prompt
...
from mini_sglang_mlx.attention import AttnBackend
from mini_sglang_mlx.core import Batch, BatchPhase, Context, Req, use_ctx
from mini_sglang_mlx.kvcache.mha_pool import MHAKVCache
from mini_sglang_mlx.models.dflash import load_dflash_draft_model
...
class DflashEngine(SpecEngine):
    ...
    ``mini_sglang_mlx_kernel.paged_prefill_attention`` via the
    ``is_cross_attention`` / ``sliding_window_size`` kwargs, so a
    standard :class:`~mini_sglang_mlx.attention.backend.AttnBackend` serves
    the draft as-is.
    """

    #: Registry key in :data:`mini_sglang_mlx.engine.SPEC_ALGOS`, and the
    ...
    def _calibrate_draft(
        ...
        Args mirror :meth:`mini_sglang_mlx.models.dflash.Model.calibrate`:
    ...
    def _run_prefill(self, batch: Batch) -> SpecForwardOutput:
        from mini_sglang_mlx.scheduler.prefill import ChunkedReq  # local: avoid cycle
