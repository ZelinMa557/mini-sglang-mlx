...
from mini_sglang_mlx.scheduler import Scheduler, SchedulerConfig
...
class LLM(Scheduler):
    def __init__(self, model_path: str, dtype: mx.DType = mx.bfloat16, **kwargs):
    ...
    def _tokenize_one(self, prompt: List[int] | str) -> mx.array:
        if isinstance(prompt, str):
            input_ids: mx.array = mx.array(  # type: ignore
                self.tokenizer.encode(prompt, return_tensors="np")  
            )
            return input_ids.view(-1).to(mx.int32)
        else:
            return mx.array(prompt, dtype=mx.int32)
