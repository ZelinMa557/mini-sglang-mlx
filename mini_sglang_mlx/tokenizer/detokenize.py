...
from mini_sglang_mlx.message import DetokenizeMsg
...
class DetokenizeManager:
    ...
    def detokenize(self, msgs: List[DetokenizeMsg]) -> List[str]:
        """Turn one batch of token messages into per-message text deltas.

        Messages are applied one at a time, in order: a single uid can
        appear several times in one batch (speculative decoding commits
        up to ``K + 1`` tokens per iter), while ``surr_offset`` /
        ``read_offset`` only advance once a message's text is known to
        be complete.  A later message must therefore take its ranges
        against the state the earlier ones left behind — reading them
        all up front lets message ``k`` re-decode and re-emit the whole
        prefix of the batch so far.
        """
        ...
        for msg in msgs:
            ...
            surr_str = self.tokenizer.decode(
